...
void readFrame(cv::Mat* frame){
    try{
        ...
        printf("Ha ocurrido un error (ERROR: %s) al leer el frame\n", str.c_str());
    }
}
    ...
    try{
        if(!frames.empty()){
            ...
            if(rec_Button->getState()){
                //printf("State: %d", rec_Button->getState());
                //resize(tmp, tmp, cv::Size(REC_WIDTH, REC_HEIGHT));
                //Sleep(1000/24);
                recFrame(tmp);
            }
...
void recFrame(cv::Mat &frame){
    ...
    try{
        video.write(frame);
        cv::putText(frame, "Grabando", cv::Point(10, frame.rows / 10), //top-left position
        cv::FONT_HERSHEY_TRIPLEX, 1.0, CV_RGB(255, 0, 0), 2);
    }
    catch(std::string str){
        printf("Hubo un error al grabar. Error: %s", str.c_str());
    }
}
